...
std::string ReadBy(std::string fileExtension) {
...
std::string FAT32::BYTEFileAttribute(std::vector<std::string> mainEntry) {
	std::string hexa = LittleEndian(mainEntry, "B", 1);
	return hexa;
}

std::string FAT32::FileAttribute(std::string hexa) {
	if (hexa == "21") {
		return "Read Only";
	}
	else if (hexa == "22") {
